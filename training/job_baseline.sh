#!/bin/bash
#SBATCH --job-name=sr_baseline
#SBATCH --output=logs/sr_baseline.o%j
#SBATCH --error=logs/sr_baseline.e%j
...
mkdir -p logs
source env.sh
srun python training/train_baseline.py --target-cache cache_ssha_wh.npy \
     --out runs/baseline_whitened "$@"
