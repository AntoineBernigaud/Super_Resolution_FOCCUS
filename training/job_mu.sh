#!/bin/bash
#SBATCH --job-name=sr_mu
#SBATCH --output=logs/sr_mu.o%j
#SBATCH --error=logs/sr_mu.e%j
...
mkdir -p logs
source env.sh
# Must run BETWEEN the two training stages: stage 2 trains on r = y - mu.
srun python training/precompute_mu.py --ckpt runs/baseline_whitened/best.pt \
     --out mu_whitened.npy "$@"
